...
class PZ_HumanRig_RemoveAllClothingItems(Operator):
    ...
    def execute(self, context):
        ...
        # self.remove_all_objs_from_col(bpy.data.collections.get("COL-PZ_Human_Male_Clothes" + instance_str))
        # self.remove_all_objs_from_col(bpy.data.collections.get("COL-PZ_Human_Female_Clothes" + instance_str))
         
        for i in range(len(m_list)):
            bpy.ops.zomboid.remove_clothing_model()

        # Remove all existing prop meshes
        # self.remove_all_objs_from_col(bpy.data.collections.get("COL-PZ_Human_Male_Props" + instance_str))
        # self.remove_all_objs_from_col(bpy.data.collections.get("COL-PZ_Human_Female_Props" + instance_str))
